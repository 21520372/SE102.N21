//Anim Sprite program source code file

#include "game.h" // Đưa vào file header của chương trình

LPDIRECT3DTEXTURE9 bill_run, bill_stand, bill_swim, bill_jump; // Tạo mảng các pointer tới các hình ảnh con mèo
LPDIRECT3DSURFACE9 background; // Tạo background
SPRITE Run,Stand,Swim,Jump; // Khai báo biến kiểu SPRITE để lưu trữ thông tin về con mèo
LPD3DXSPRITE sprite_handler;
HRESULT result;
long start = GetTickCount(); // Lưu lại thời điểm bắt đầu chương trình
//khởi tạo game
int GameInit(HWND hWnd)
{
    ...
    result = D3DXCreateSprite(d3ddev, &sprite_handler); // Khởi tạo sprite handler để vẽ sprite
    if (!SUCCEEDED(result))
    {
        return 0; // Nếu không khởi tạo được thì trả về 0 để báo lỗi
    }
    bill_run = LoadTexture(L"bill_move.png", D3DCOLOR_XRGB(255, 0, 255), SCALE_CHARACTER); // Load hình ảnh của sprite vào biến contra_image với màu nền magenta
    if (bill_run == NULL)
    {
        return 0; // Nếu không load được hình ảnh thì trả về 0 để báo lỗi
    }
    bill_stand = LoadTexture(L"bill_stand.png", D3DCOLOR_XRGB(255, 0, 255), SCALE_CHARACTER); // Load hình ảnh của sprite vào biến contra_image với màu nền magenta
    if (bill_stand == NULL)
    {
        return 0; // Nếu không load được hình ảnh thì trả về 0 để báo lỗi
    }
    bill_swim = LoadTexture(L"bill_in_water.png", D3DCOLOR_XRGB(255, 0, 255), SCALE_CHARACTER); // Load hình ảnh của sprite vào biến contra_image với màu nền magenta
    if (bill_swim == NULL)
    {
        return 0; // Nếu không load được hình ảnh thì trả về 0 để báo lỗi
    }
    bill_jump = LoadTexture(L"bill_jump.png", D3DCOLOR_XRGB(255, 0, 255), SCALE_CHARACTER); // Load hình ảnh của sprite vào biến contra_image với màu nền magenta
    if (bill_swim == NULL)
    {
        return 0; // Nếu không load được hình ảnh thì trả về 0 để báo lỗi
    }
    //Khởi tạo background
    background = LoadSurface(L"green.png", NULL); // Load hình nền cho background
    if (background == NULL)
    {
        return 0; // Nếu không load được hình nền thì trả về 0 để báo lỗi
    }
    ...
    Jump.movex = 0;
    Jump.movey = 10;
    return 1; // Trả về giá trị 1 để báo hiệu quá trình khởi tạo đã hoàn tất
}
